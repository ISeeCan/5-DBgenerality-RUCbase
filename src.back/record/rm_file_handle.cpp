...
Rid RmFileHandle::insert_record(char* buf, Context* context) {
    ...
    if (i == -1) {
        // 如果当前页面已满，需要查找下一个未满页面来插入记录
        for (int pageNo = file_hdr_.first_free_page_no; pageNo != -1; pageNo = pagehandle.page_hdr->next_free_page_no) {
            pagehandle = fetch_page_handle(pageNo);
            i = Bitmap::first_bit(0, pagehandle.bitmap, file_hdr_.num_records_per_page);
            if (i != -1) {
                break;  // 找到了未满的页面，退出循环
            }
        }
    ...
    memcpy(slot, buf, file_hdr_.record_size);  // 将数据从 buf 复制到插槽
    ...
    return Rid{pagehandle.page->get_page_id().page_no, i};  // 返回表示插入记录位置的 Rid
}
...
void RmFileHandle::delete_record(const Rid& rid, Context* context) {
    ...
    if(pagehandle.page_hdr->num_records==file_hdr_.num_records_per_page-1)// 如果页面中的记录数为每页最大记录数减1，说明刚刚是满的，释放页面句柄
    {
...
RmPageHandle RmFileHandle::create_new_page_handle() {
    ...
    newpageid->fd = fd_;
    Page *newpage = buffer_pool_manager_->new_page(newpageid);// 创建一个RmPageHandle对象，将文件头和新页面传递给它
    RmPageHandle newPageHandle = RmPageHandle(&file_hdr_, newpage);// 创建一个RmPageHandle对象，将文件头和新页面传递给它
    newPageHandle.page_hdr->next_free_page_no = file_hdr_.first_free_page_no;// 设置新页面的下一个可用页面编号为文件头的first_free_page_no
...
RmPageHandle RmFileHandle::create_page_handle() {
    ...
    if (file_hdr_.first_free_page_no == -1)// 如果文件头的first_free_page_no为-1，说明没有空闲页面，需要创建新的页面句柄
    {
...
void RmFileHandle::release_page_handle(RmPageHandle&page_handle) {
    ...
    page_handle.page_hdr->next_free_page_no = file_hdr_.first_free_page_no;// 设置页面头的下一个可用页面编号为文件头的first_free_page_no
    file_hdr_.first_free_page_no = page_handle.page->get_page_id().page_no;// 更新文件头的first_free_page_no为页面的页号
}